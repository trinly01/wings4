...
// import feathers from '@feathersjs/client/dist/feathers.js'
...
export default (host) => {
  ...
  app.wingsService = (serviceName, query = {}, conf = {}) => {
    ...
    // wings.disconnectCallback = ()
    wings.onDisconnect = (callback) => {
      wings.status = 'offline'
      wings.log(`${serviceName}.onDisconnect`)
      if (wings.disconnectCallback) wings.disconnectCallback()
    }
    app.io.on('disconnect', wings.onDisconnect)

    // wings.connectCallback = ()
    wings.onConnect = (callback) => {
      // if (wings.data.length) {
      wings.log(`${serviceName}.onConnect`)
      wings.reset()
      if (wings.connectCallback) wings.connectCallback()
      // }
    }
